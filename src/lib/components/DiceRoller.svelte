<script lang="ts">
    ...
    onMount(() => {
        ...
        diceBox.onRollComplete = (results: any) => {
            dice_values = results.map((dice: {rolls: {value: number}[]}) => {
                return dice.rolls.map(
                    (roll) => roll.value
                ).join(" + ")
            }).join(" + ");
            roll_result = results.reduce((sum: number, dice: {rolls: {value: number}[]}) => {
                const diceTotal = dice.rolls.reduce((acc, roll) => acc + roll.value, 0);
                return sum + diceTotal;
            }, 0);

            // Celebration
            if (single_roll && (rolls[0] === "1d20" || rolls[0] === "d20") && roll_result === 20) {
                confetti_function({
                    particleCount: 100,
                    spread: 70,
                    origin: { y: 2 }
            ...
            if (single_roll && (rolls[0] === "1d20" || rolls[0] === "d20") && roll_result === 1) {
                confetti_function({
                    particleCount: 100,
                    spread: 70,
                    scalar: 4,
